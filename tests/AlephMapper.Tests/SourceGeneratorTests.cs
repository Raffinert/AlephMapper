...
public class SourceGeneratorTests
{
    ...
    public async Task GeneratedMembersUseProjectNullablePolicy()
    {
        var cases = new[]
        {
            new { Directive = "#nullable disable", Annotations = true, ProjectDefault = NullableContextOptions.Enable },
            new { Directive = "#nullable enable", Annotations = true, ProjectDefault = NullableContextOptions.Enable },
            new { Directive = "#nullable enable", Annotations = false, ProjectDefault = NullableContextOptions.Disable },
            new { Directive = "#nullable enable", Annotations = false, ProjectDefault = NullableContextOptions.Warnings },
            new { Directive = "#nullable disable", Annotations = true, ProjectDefault = NullableContextOptions.Annotations }
        };
    ...
    public async Task InlinedHelpersUseTheProjectNullablePolicy()
    {
        const string source = """
            ...
            {
                [Projectable]
                public static Destination Map(Source source) => LegacyHelper.Map(source);
            }
            ...
            public sealed class Destination { public string Name { get; set; } = string.Empty; }
            """;
        var references = await ReferenceAssemblies.Net.Net90.ResolveAsync(LanguageNames.CSharp, CancellationToken.None);
        var compilation = CSharpCompilation.Create(
        ...
        await Assert.That(mapperSource).Contains("source => new global::NullablePolicyFixture.Destination");
        await Assert.That(mapperSource.Split("#nullable restore", StringSplitOptions.None)).Count().IsEqualTo(2);
        await Assert.That(mapperSource).DoesNotContain("#nullable disable");
        await Assert.That(outputCompilation.GetDiagnostics().Where(diagnostic =>
